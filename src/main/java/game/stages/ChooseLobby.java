...
public class ChooseLobby {
  ...
  /**
   * * Initialize ChooseLobby Menu. Will load the texture files and generate the
   * basic menu parts. This
  ...
  public static void init(Loader loader) {
    ...
    lobbyOverview = new GuiTexture(loader.loadTexture("lobbyOverview"), new Vector2f(0, -0.040741f),
            new Vector2f(0.554167f, 0.757804f), 1);

    titel = new GuiTexture(loader.loadTexture("availableLobbiesType"),
            new Vector2f(-0.053125f, 0.446296f), new Vector2f(0.379167f, 0.052778f), 1);
    ...
    for (int i = 0; i < joinY.length; i++) {
      join[i] = new MenuButton(
              loader, "join_norm", "join_hover", new Vector2f(0.391667f, joinY[i]),
              new Vector2f(0.082365f, .069444f));
    }
  }

  /**
   * Game Loop while the stage is active. This runs every frame
   * as long as the InLobby-Menu is active.
