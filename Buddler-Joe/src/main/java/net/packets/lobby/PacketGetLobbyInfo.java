...
/**
 * A Packet that gets send from the Client to the Server, to get Information about his current
 * lobby. Packet-Code: LOBGI
 *
...
public class PacketGetLobbyInfo extends Packet {
   ...
   * {@link Packet#addError(String)}. Constructs a {@link PacketCurLobbyInfo}-Packet that contains
   * the names of all clients that are in the current lobby of the sender, or in the case of an
   * error, a suitable errormessage. (names are separated by "║") If there are no errors "OK" gets
   * added to the String of the {@link PacketCurLobbyInfo}-Packet Sends the {@link
   * PacketCurLobbyInfo}-Packet to the client that has send this packet.
   */
  @Override
