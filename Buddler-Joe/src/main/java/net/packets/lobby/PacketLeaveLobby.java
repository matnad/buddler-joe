...
public class PacketLeaveLobby extends Packet {
    ...
    /**
     * Constructor that will be used by the Client to build the Packet.
     * There are no parameters necessary here, since the Packet has no real content(only a Type, LOBLE).
     */
     ...
     * If there are no errors, the client gets removed from his current lobby.
     * Constructs a {@link PacketLeaveLobbyStatus}-Packet that contains either "OK" if the leave-attempt  was
     * successful, or in the case of an error, a suitable errormessage.
     * Sends the {@link PacketLeaveLobbyStatus}-Packet to the client that tried to leave a lobby.
     * If no errors:
     * Creates and sends a {@link PacketLobbyOverview}-Packet to all clients that are not in a Lobby at the moment
     * (including the client that has just left his lobby).
