...
public class ParticleRenderer {
	...
	private static final float[] VERTICES = {-0.5f, 0.5f, -0.5f, -0.5f, 0.5f, 0.5f, 0.5f, -0.5f}; //A simple quad
	...
	protected void render(Map<ParticleTexture, List<Particle>> particles, Camera camera){
		...
		for (ParticleTexture particleTexture : particles.keySet()) {
			//Loop over particle types and check if we want additive blending or not (on a type per type basis)
	...
	/*
	 * This calculation needs to be done for every particle in every frame and is currently done all on the CPU.
	 * TODO: It might be more effective to move (part of) this onto the shader?
	 * If we ever run into performance problems, this is one of the functions we can probably optimize, but I want to
