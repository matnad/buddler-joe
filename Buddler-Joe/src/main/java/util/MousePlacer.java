...
public class MousePlacer {
    ...
    public static void update(Camera camera) {
        ...
        if(mode == modes.Z3OFFSET.getMode()) {
            ...
            if (intersections.size() == 0) {
                ...
                entity.setPosition(new Vector3f(wallIntersection.x, wallIntersection.y, wallIntersection.z-2));
            ...
            if(InputHandler.isMousePressed(GLFW_MOUSE_BUTTON_1) && !doesCollide(3)) {
                //Bind torchs to a block if placed in a block.
    ...
    private static Vector3f getZ3Intersection(Camera camera) {
        float distance = Intersectionf.intersectRayPlane(camera.getPosition(), InputHandler.getMouseRay(),
