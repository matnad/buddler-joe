...
class GenerateWorld {
    ...
    public static void generateTerrain(Loader loader) {
        ...
        for (int i = 0; i < 200; i++) {
            if (i % 3 == 0) {
                Game.addEntity(new Entity(fern, random.nextInt(4), getNextRandomVector3f(aboveGround), 0, 0, 0, .9f));
                Game.addEntity(new Entity(tree2, getNextRandomVector3f(aboveGround), 0, random.nextFloat() * 360, 0,
    ...
    static void generateBlocks(Loader loader) {
        ...
        float size = 3; //If this is not 3, you need to use the full block constructor and modify the Block Master or the Block Files. Just use 3 for now.
    ...
    /**
     * Get a random point on the terrain surface for a 200x200 standard terrain, respecting height maps.
     * Only use this for terrains with height map, otherwise simply randomize two coordinates on a plane.
